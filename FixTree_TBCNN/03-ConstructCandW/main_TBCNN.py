...
from diffAST.main import create
import gl
...
def ConstructNodes(ast, name, parent, pos, nodes, leafs):
    ...
    Node = Token.token(name, gl.numFea * ast.type, \
                       parent, pos)
...
def InitNetbyText(inPATH='', key_file= ''):
    # parser = pycparser.c_parser.CParser()
    # ast = parser.parse(text=text)  # Parse code to AST
    ast = create(inPATH, key_file).getHeadNode()
